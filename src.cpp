...
int main()
{
	...
	while (program)
	{
		...
		program = PruebaUsuario.CallLogin(useractual);
		if (program == false)
		{
			break;
		}
		role = PruebaUsuario.GetRole(useractual);
		Pruebas.Clear();
		...
		case 1:
			...
			case 1:
				if (role == 4)
				{
					PruebaUsuario.callRegister();
				}
				else
				{
					cout << "No tiene permiso para ver esta funcion" << endl;
				}
			}
		default:
			break;
